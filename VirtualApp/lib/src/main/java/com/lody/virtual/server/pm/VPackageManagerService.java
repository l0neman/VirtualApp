...
public class VPackageManagerService implements IPackageManager {
  ...
  @Override public List<String> getSharedLibraries(String packageName) {
  ...
  @Override public int checkPermission(String permName, String pkgName, int userId) {
  ...
  @Override public PackageInfo getPackageInfo(String packageName, int flags, int userId) {
    checkUserId(userId);
    synchronized (mPackages) {
      VPackage p = mPackages.get(packageName);
      if (p != null) {
  ...
  private PackageInfo generatePackageInfo(VPackage p, PackageSetting ps, int flags, int userId) {
    ...
    PackageInfo packageInfo = PackageParserEx.generatePackageInfo(p, flags, ps.firstInstallTime, ps.lastUpdateTime, ps.readUserState(userId), userId);
    if (packageInfo != null) {
      return packageInfo;
    }
    return null;
  ...
  @Override public ActivityInfo getActivityInfo(ComponentName component, int flags, int userId) {
    checkUserId(userId);
    flags = updateFlagsNought(flags);
  ...
  public boolean activitySupportsIntent(ComponentName component, Intent intent, String resolvedType) {
    synchronized (mPackages) {
      ...
      for (int i = 0; i < a.intents.size(); i++) {
        if (a.intents.get(i).filter.match(intent.getAction(), resolvedType, intent.getScheme(), intent.getData(), intent.getCategories(), TAG) >= 0) {
          return true;
        }
      }
  ...
  @Override public ActivityInfo getReceiverInfo(ComponentName component, int flags, int userId) {
    checkUserId(userId);
    flags = updateFlagsNought(flags);
    synchronized (mPackages) {
      VPackage p = mPackages.get(component.getPackageName());
  ...
  @Override public ServiceInfo getServiceInfo(ComponentName component, int flags, int userId) {
    checkUserId(userId);
    flags = updateFlagsNought(flags);
    synchronized (mPackages) {
      VPackage p = mPackages.get(component.getPackageName());
  ...
  @Override public ProviderInfo getProviderInfo(ComponentName component, int flags, int userId) {
    checkUserId(userId);
    flags = updateFlagsNought(flags);
    synchronized (mPackages) {
      VPackage p = mPackages.get(component.getPackageName());
  ...
  private ResolveInfo findPreferredActivity(Intent intent, String resolvedType, int flags,
                                            List<ResolveInfo> query, int priority) {

    try {
      Class clazz = Class.forName("com.virtual.helper.VALibHelper");
      Method method = clazz.getDeclaredMethod("findPreferredActivity", Intent.class,
  ...
  @Override public VParceledListSlice<PackageInfo> getInstalledPackages(int flags, int userId) {
    checkUserId(userId);
  ...
  @Override public ProviderInfo resolveContentProvider(String name, int flags, int userId) {
    checkUserId(userId);
    flags = updateFlagsNought(flags);
    synchronized (mPackages) {
  ...
  @Override public ApplicationInfo getApplicationInfo(String packageName, int flags, int userId) {
    checkUserId(userId);
    flags = updateFlagsNought(flags);
    synchronized (mPackages) {
      VPackage p = mPackages.get(packageName);
  ...
  @Override public int getPackageUid(String packageName, int userId) {
    checkUserId(userId);
    synchronized (mPackages) {
      VPackage p = mPackages.get(packageName);
      if (p != null) {
  ...
  @Override public List<String> querySharedPackages(String packageName) {
    synchronized (mPackages) {
      VPackage p = mPackages.get(packageName);
  ...
  @Override public IBinder getPackageInstaller() {
  ...
  private final class ActivityIntentResolver extends IntentResolver<VPackage.ActivityIntentInfo, ResolveInfo> {
    ...
    List<ResolveInfo> queryIntentForPackage(Intent intent, String resolvedType, int flags, ArrayList<VPackage.ActivityComponent> packageActivities, int userId) {
      ...
      ArrayList<VPackage.ActivityIntentInfo[]> listCut = new ArrayList<VPackage.ActivityIntentInfo[]>(N);
    ...
    @Override protected VPackage.ActivityIntentInfo[] newArray(int size) {
      return new VPackage.ActivityIntentInfo[size];
    }

    @Override protected boolean isFilterStopped(VPackage.ActivityIntentInfo filter) {
      return false;
    }
    ...
    @Override protected void sortResults(List<ResolveInfo> results) {
    ...
    @Override protected Object filterToLabel(VPackage.ActivityIntentInfo filter) {
  ...
  private final class ServiceIntentResolver extends IntentResolver<VPackage.ServiceIntentInfo, ResolveInfo> {
    ...
    @Override protected VPackage.ServiceIntentInfo[] newArray(int size) {
      return new VPackage.ServiceIntentInfo[size];
    }

    @Override protected boolean isFilterStopped(VPackage.ServiceIntentInfo filter) {
      return false;
    }
    ...
    @Override protected Object filterToLabel(VPackage.ServiceIntentInfo filter) {
